...
- *Затем, в окне терминала запустим программу с помощью встроенного интерпретатора*:`python3 test.py`
## *Работа с файловой системой.Перенаправление ввода-вывода и конвейеры*
- *Однако текст файла можно написать и без использования блокнота. Для этого применим команду:`echo print("data") > data.py`.*
- *Затем с помощью интерепретатора можем проверить наш код.*
...
- *Для автоматизации создания файлов можно использовать циклические и логические конструкции. Например,ветвление может выглядеть таким образом*
``` bash 
if команда1
then команды
elif команда2 
