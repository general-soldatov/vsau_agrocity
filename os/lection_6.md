...
- **Способы организации**:
    ...
    - Такая организация удобно,когда файлов мало и один пользователь.Это стркуктура требует постоянного хранения.В логической файловой системе появляются 2 различных типа файлов:директории и все остальные -регулярные файлы.
# Файлы и файловые системы
### Логическая файловая система
- **Способы организации**:
    - **Двухуровневая директория**- в этом случае у нас есть главная директория ("Madter files"),в которой лежат директории отдельных пользователей,у каждого из которых есть своя"куча",организованная в виде одноуровневой директории("User files directory").
...
- [ ] Также в эту таблицу можно разместить разместить размер файла и др.атрибуты->индексный узел.Одного в этом случае индексный узел может иметь переменный размер.
- [ ] Решить проблему может помочь многоуровневая индексация.
# Файлы и файловые системы
### Физические файлы и файловая система.
- **Метод выделения блоков.Смешанная индексация.**
...
- Функции:
    ...
    - Обеспечение выполнение операций над файласм и логическими файловыми системами.
...
- Распределение внешней памяти между файлами.Учёт занятого и свободного пространства внешней памяти.
    ...
    - Файловая подсистема ОС ведёт учет свободных логических блоков:
    - [ ] С помощью битого вектора
...
- Идентификация файлов.Связывание имени с его расположением на внешнем устройстве.
    ...
    - Считываем содержимое корней директории,находим запись,соотвествующию имена"/a",убеждаемся,что это его директория,и определяем расположением расположение этого файла на диске.
